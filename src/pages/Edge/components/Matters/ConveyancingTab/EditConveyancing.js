...
import { editConveyance } from "../../../apis";
...
const EditConveyancing = (props) => {
  ...
  useEffect(() => {
    if (props.data) {
      ...
      initialStateRef.current = props.data;
    }
  }, [props.data]);
  ...
  const handleSubmit = async () => {
    ...
    if (invalid) {
      ...
      try {
        const { data } = await editConveyance(d);
        if (data.success) {
          if (props.refresh) {
            props.refresh();
            setTimeout(() => {
    ...
    <Fragment>
      <div>
        {/* <CustomToastWindow
          ...
          btn1={'Cancel'}
          btn2='Update'
          heading='Edit Conveyancing Details'
...
export default EditConveyancing;
