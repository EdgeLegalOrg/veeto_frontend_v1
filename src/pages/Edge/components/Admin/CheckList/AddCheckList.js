...
const AddCheckList = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!newData.name?.trim()) {
      ...
      setLoading(true);
      try {
        const { data } = await addNewChecklist(newData);
        if (data.success) {
          if (props.refresh) {
            props.refresh();
          }
            ...
            <tr key={task.id}>
              ...
              >
                <Input
                  ...
                  onChange={() => {}}
                />
                <label className="mb-0 mx-2">{task.title}</label>
              </td>
            </tr>
          ))}
        </tbody>
