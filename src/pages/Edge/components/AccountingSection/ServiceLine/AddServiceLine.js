...
import { addServiceLine } from "../../../apis";
...
const AddServiceLine = (props) => {
  const { setShowAdd, refresh, billingList, typeList, taxTypeList } = props;
  ...
  const handleAddNew = async () => {
    try {
      const { data } = await addServiceLine(formData);
      setShowAdd(false);
      if (data.success) {
        refresh();
      } else {
  ...
  return (
    <Fragment>
      ...
      <div className="d-flex align-items-center justify-content-end p-2 border-top mt-4">
        ...
        <Button color="success" className="mx-1" onClick={() => handleAddNew()}>
          Save
        </Button>
      </div>
      {/* </CustomToastWindow> */}
